...
"""Miscellaneous helper functions needed by autowrap.py."""
...
class Indenter:
    """Callable context manager for tracking string indentation levels.
...
def indent(s, n=1, indent_str="  "):
    ...
    padding = n * indent_str
    return "".join((padding + line) if line.lstrip() else line for line in s.splitlines(True))
...
class UniqueOrderedDict(collections.OrderedDict):
    ...
    def __setitem__(self, k, v):
        ...
        if existing_v is None:
            ...
            raise ValueError(f"Key '{k}' already exists with a different value.")
...
def try_coerce_to_num(s, try_types=(int, float)):
    """Tries to coerce a string to a Python numeric type, returns None if empty."""
    if not s:
        return None
...
def comment_line(string, width=79, fill_char="-"):
    ...
    return f"# {string:{fill_char}^{width-2}}\n"
